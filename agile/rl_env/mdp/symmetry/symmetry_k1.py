...
def lr_mirror_K1(
    ...
    """Left-right symmetry augmentation for the Booster K1 robot.
    ...
    if actions is not None:
        mirrored_actions = mirror_actions_K1(actions, env)
...
def mirror_actions_K1(actions: torch.Tensor, env: ManagerBasedRLEnv) -> torch.Tensor:
    """Left-right mirroring of the actions. Can be a subset of the joints as defined in the action manager."""

    mirrored_indices, neg_indices = resolve_joint_names_k1(
...
def mirror_joints_K1(actions: torch.Tensor, env: ManagerBasedRLEnv) -> torch.Tensor:
    """Left-right mirroring of all the joints of the Booster K1 robot."""
...
def mirror_bodies_K1(bodies: torch.Tensor, env: ManagerBasedRLEnv) -> torch.Tensor:
    """Left-right mirroring of **all** the bodies of the Booster K1 robot."""

    mirrored_indices = resolve_body_names_k1(tuple(env.unwrapped.scene.articulations["robot"].body_names))
...
def resolve_joint_names_k1(action_joint_names: tuple[str, ...]) -> tuple[list[int], list[int]]:
    ...
    neg_indices = []
    neg_joint_indicators = ["Roll", "Yaw", "yaw"]
...
@lru_cache(maxsize=10)
def resolve_body_names_k1(body_names: tuple[str, ...]) -> list[int]:
    """Resolve the joint names to indices.
...
def mirror_actuator_gains(obs: torch.Tensor, env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    """
    mirrored_indices, _ = resolve_joint_names_k1(tuple(env.unwrapped.scene.articulations["robot"].joint_names))
...
OBS_TO_MIRROR: dict[str, Callable] = {
    ...
    "joint_pos": mirror_joints_K1,
    "joint_vel": mirror_joints_K1,
    "actions": mirror_actions_K1,
    "controlled_joint_pos": mirror_actions_K1,
    "controlled_joint_vel": mirror_actions_K1,
    ...
    "action_smoothing": mirror_actions_K1,
    "contact_forces": mirror_bodies_K1,
}
